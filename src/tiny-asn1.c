...
uint32_t fetch_data_length(const uint8_t *in, uint32_t inlen)
{
  uint32_t x, z;
...
int32_t der_object_count(const uint8_t* in, uint32_t inlen)
{
  ...
  if((*in & 0x20) != 0) {
    ...
    while(children_length < in_data_length) {
      const uint8_t* child = (in+in_data_offset);
	...
	return -1;
      ...
      int32_t child_objects = der_object_count(child, child_length);
      if(child_objects == -1)
        return -1;
...
int32_t der_decode(const uint8_t *in, uint32_t inlen, asn1_tree *out, asn1_tree* out_objects, unsigned int out_objects_count)
{
  //Check the input parameters
  ...
  //If more data is passed, than is encoded (inlen >  (in_encoded_length + in_data_offset)), that data is ignored
  const uint8_t* in_data = (in + in_data_offset);
  uint32_t in_data_length = in_encoded_length - in_data_offset;
  ...
  if((*in & 0x20) != 0) {
    ...
    while(children_length < in_data_length) {
      ...
      int32_t child_objects = der_object_count(child, child_length);
      if(child_objects < 0 || child_objects > (int)out_objects_count) {
        return -6;
      }
...
int32_t der_encode(asn1_tree* asn1, uint8_t* encoded, uint32_t encoded_length)
{
  ...
  *encoded = asn1->type;
  ...
  if((asn1->type & 0x20) != 0) {
    ...
    //Just copy the data
    if(asn1->length > 0) {
      if(asn1->length <= encoded_length)
        memcpy(encoded, asn1->data, asn1->length);
      else
        // The target buffer is not large enough.
        return -1;
    }
  }
